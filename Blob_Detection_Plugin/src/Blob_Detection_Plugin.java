 ...
 * @author Laura Goold
 */
public class Blob_Detection_Plugin implements PlugInFilter{
    ...
    public void run(ImageProcessor ip) {
        ...
        Color[][] colours = new Color[image.getHeight()][image.getWidth()];
        int[][] blobVals = new int[image.getHeight()][image.getWidth()];
        int curBlobCount = 0;
        ArrayList<Integer> blobConnectionList = new ArrayList<Integer>();
        
        for(int i = 0; i < image.getHeight(); i++){
            for(int j = 0; j < image.getWidth(); j++){
                colours[i][j] = new Color(image.getRGB(j, i));                
            }
        }
        ...
        for(int i=0; i<image.getHeight(); i++)
        {
            for(int j=0; j<image.getWidth(); j++)
            {
                        ...
                        && colours[i][j].getBlue() >= blueMin && colours[i][j].getBlue() <= blueMax)
                {
                    ...
                    surrounding[3] = (i > 0 && j < image.getWidth()-1) ? blobVals[i-1][j+1] : 0;    //Top-Right                   
        ...
        int blobsFound = 0;
        ArrayList<Tuple<Integer, Integer>> associations = new ArrayList<Tuple<Integer, Integer>>();//issue
        ArrayList<ArrayList<Integer>> includeList = new ArrayList<ArrayList<Integer>>();//issue
        ...
        ArrayList<Color> colorList = new ArrayList<Color>(); //issue
        ...
        for(int i = 0; i < image.getHeight(); i++){
            for(int j = 0; j < image.getWidth(); j++){
                if(blobVals[i][j] != 0){
                    int rgb = colorList.get(blobVals[i][j]-1).getRGB();
                    //ip.putPixel(i, j, rgb);
                    image.setRGB(j, i, colorList.get(blobVals[i][j]-1).getRGB());
